...
const VideoGallery = () => {
    ...
    // Auto scroll
    useEffect(() => {
        if (!scrollContainerRef.current || isDragging || videos.length === 0) return;
            ...
            <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-16">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
                        ...
                        <h3 className="text-xl font-semibold text-gray-800 mb-2">কোনো ভিডিও পাওয়া যায়নি</h3>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="py-12 bg-black">
                ...
                >
                    <div className="flex gap-4 sm:gap-6" style={{ width: 'max-content' }}>
                        {infiniteVideos.map((video, index) => (
                            <div
                                key={`${video._id}-${index}`}
                                className="shrink-0 w-64 sm:w-80 bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer"
            ...
            {/* Modal */}
            {selectedVideo && (
                <div className="fixed inset-0 bg-black/40 bg-opacity-90 flex items-center justify-center z-50 p-4" onClick={closeModal}>
                    <div className="relative w-full max-w-4xl" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-white text-center mt-4 text-lg font-medium">{selectedVideo.caption}</p>
                    </div>
                </div>
            )}
        </div>
