...
const PaymentTypes = ({ onBack }) => {
    ...
    const fetchPaymentTypes = async () => {
        try {
            ...
            showSweetAlert('error', 'পেমেন্ট টাইপ লোড করতে সমস্যা হয়েছে');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (paymentTypeId) => {
        const result = await Swal.fire({
            ...
            text: "এই পেমেন্ট টাইপটি ডিলিট হয়ে যাবে!",
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                const response = await axiosInstance.delete(`/payment-types/${paymentTypeId}`);
                if (response.data.success) {
                    showSweetAlert('success', 'পেমেন্ট টাইপ সফলভাবে ডিলিট হয়েছে');
                    fetchPaymentTypes();
                }
            } catch (error) {
                console.error('Error deleting payment type:', error);
                showSweetAlert('error', 'পেমেন্ট টাইপ ডিলিট করতে সমস্যা হয়েছে');
            }
        }
    };

    const handleToggleStatus = async (paymentTypeId, currentStatus) => {
        try {
            const response = await axiosInstance.patch(`/payment-types/${paymentTypeId}/toggle-status`);
            if (response.data.success) {
                ...
                <div className="max-w-full mx-auto">
                    {loading ? (
                        <div className="flex justify-center items-center py-12">
                            <Loader />
                            <p className="text-gray-600 ml-3">পেমেন্ট টাইপ লোড হচ্ছে...</p>
                        </div>
                    ) : paymentTypes.length === 0 ? (
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 text-center">
                            <div className="text-4xl text-gray-400 mb-3">💳</div>
                            <h3 className="text-lg font-semibold text-gray-800 mb-2">
                                কোন পেমেন্ট টাইপ পাওয়া যায়নি
                            </h3>
                            <p className="text-gray-600 text-sm mb-4">
                                নতুন পেমেন্ট টাইপ তৈরি করুন
                            </p>
                            <button
                                onClick={handleAddNew}
                        ...
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                            ...
                            <div className="px-6 py-4 border-b border-gray-200">
                                <h2 className="text-lg font-semibold text-gray-800">
                                    পেমেন্ট টাইপ তালিকা ({paymentTypes.length}টি)
                                </h2>
                            </div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500">
                                                            তৈরি: {new Date(type.createdAt).toLocaleDateString('bn-BD')}
                                                        </p>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
