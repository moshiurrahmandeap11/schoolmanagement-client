...
// Import all the print components (you'll need to create these)
import AdmitCard from '../../Print/AdmitCard/AdmitCard';
import Marksheets from '../ExaminationManagement/Marksheet/Marksheet';
...
const PrintManagement = () => {
    ...
    const printOptions = [
        {
            id: 'marksheets',
            title: 'Marksheets',
            subtitle: 'Print Marksheets',
            icon: FaFileAlt,
            color: 'blue',
            description: 'শিক্ষার্থীদের মার্কশিট প্রিন্ট করুন',
            component: Marksheets
        },
        {
            id: 'admit-card',
            title: 'অ্যাডমিট কার্ড',
            subtitle: 'Print admit card',
            icon: FaIdCard,
            color: 'green',
            description: 'শিক্ষার্থীদের অ্যাডমিট কার্ড প্রিন্ট করুন',
            component: AdmitCard
        }
    ];
    ...
    const ActiveComponent = printOptions.find(opt => opt.id === activeTab)?.component;
                ...
                <div className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
                    <div className="flex items-center gap-4 p-4 sm:p-6">
                        ...
                        <h1 className="text-2xl font-bold text-gray-800">
                            {printOptions.find(opt => opt.id === activeTab)?.title || 'প্রিন্ট ব্যবস্থাপনা'}
                        </h1>
                    </div>
                </div>
            )}
                    ...
                    <div className="max-w-full mx-auto">
                        <div className="text-center mb-10">
                            ...
                            <p className="text-gray-600 text-lg">
                                আপনার প্রতিষ্ঠানের সকল প্রিন্ট সম্পর্কিত কার্যক্রম নিয়ন্ত্রণ করুন
                            </p>
                        </div>

                        {/* Print Options Grid - Responsive */}
                            ...
                            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
                                <div className="text-center mb-8">
                                    <div className="w-20 h-20 mx-auto mb-4 bg-blue-100 rounded-full flex items-center justify-center">
                                        {(() => {
                                            const option = printOptions.find(opt => opt.id === activeTab);
                                            const IconComponent = option?.icon || FaFileAlt;
                                            const colorClass = colorClasses[option?.color || 'blue'];
                                            return <IconComponent className={`text-3xl ${colorClass.icon}`} />;
                                        })()}
                                    </div>
                                    <h2 className="text-3xl font-bold text-gray-800 mb-2">
                                        {printOptions.find(opt => opt.id === activeTab)?.title}
                                    </h2>
                                    <p className="text-gray-600 text-lg">
                                        {printOptions.find(opt => opt.id === activeTab)?.subtitle}
                                    </p>
                                </div>
                                ...
                                <div className="text-center py-12">
                                    <p className="text-gray-500 text-lg">
                                        Component not found for {printOptions.find(opt => opt.id === activeTab)?.title}
                                    </p>
                                    <button
                                        onClick={handleBack}
