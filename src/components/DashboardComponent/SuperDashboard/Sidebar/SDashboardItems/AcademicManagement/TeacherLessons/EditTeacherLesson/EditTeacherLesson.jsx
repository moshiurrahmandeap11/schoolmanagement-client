...
const EditTeacherLesson = ({ lesson, onBack, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axiosInstance.put(`/teacher-lessons/${lesson._id}`, submitData, {
                headers: {
            ...
            if (response.data.success) {
                showSweetAlert('success', 'লেসন প্ল্যান সফলভাবে আপডেট হয়েছে!');
                if (onSuccess) {
                    onSuccess();
                }
                onBack();
            } else {
                showSweetAlert('error', response.data.message || 'লেসন প্ল্যান আপডেট করতে সমস্যা হয়েছে');
            }
        } catch (error) {
            console.error('Error updating lesson plan:', error);
            const errorMessage = error.response?.data?.message || 'লেসন প্ল্যান আপডেট করতে সমস্যা হয়েছে';
                ...
                <div className="max-w-4xl mx-auto">
                    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                        <div className="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                <label className="block text-gray-700 font-medium mb-2 text-sm">
                                    লেসন প্ল্যান ফাইল আপলোড (ঐচ্ছিক)
                                </label>
                                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#1e90c9] transition-colors">
                                    ...
                                    <label htmlFor="lessonFile" className="cursor-pointer">
                                        ...
                                        <p className="text-gray-600 mb-2">
                                            {formData.lessonFile 
                                                ? `নতুন ফাইল: ${formData.lessonFile.name}` 
                                                : lesson.fileName 
                                                    ? `বর্তমান ফাইল: ${lesson.fileName}` 
                                                    : 'নতুন ফাইল নির্বাচন করুন (PDF, DOC, DOCX, TXT, PPT, PPTX)'
                                            }
                                        </p>
                                ...
                                {formData.lessonFile && (
                                    <p className="text-green-600 text-xs mt-1">
                                        নতুন ফাইল নির্বাচিত হয়েছে: {formData.lessonFile.name} 
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                >
                                    ...
                                    {loading ? 'আপডেট হচ্ছে...' : 'লেসন প্ল্যান আপডেট করুন'}
                                </MainButton>
                            </div>
                        </div>
                    </form>
