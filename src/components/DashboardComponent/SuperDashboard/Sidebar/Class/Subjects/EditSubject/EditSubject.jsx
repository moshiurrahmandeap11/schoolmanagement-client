...
const EditSubject = ({ subject, onBack, onSuccess }) => {
    ...
    useEffect(() => {
        if (subject) {
            setFormData({
                ...
                writtenPassMark: subject.writtenPassMark || '',
                mcqPassMark: subject.mcqPassMark || '',
                practicalPassMark: subject.practicalPassMark || '',
                ctPassMark: subject.ctPassMark || '',
                firstPaper: subject.firstPaper || '',
                bothPapersTotalMarks: subject.bothPapersTotalMarks || '',
                bothPapersMcqPassMark: subject.bothPapersMcqPassMark || '',
                bothPapersPracticalPassMark: subject.bothPapersPracticalPassMark || '',
                bothPapersCtPassMark: subject.bothPapersCtPassMark || ''
            });
        }
        fetchClasses();
        fetchSections();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axiosInstance.put(`/subjects/${subject._id}`, formData);

            if (response.data.success) {
                showSweetAlert('success', 'বিষয় সফলভাবে আপডেট হয়েছে!');
                if (onSuccess) {
                    onSuccess();
                }
                onBack();
            } else {
                showSweetAlert('error', response.data.message || 'বিষয় আপডেট করতে সমস্যা হয়েছে');
            }
        } catch (error) {
            console.error('Error updating subject:', error);
            const errorMessage = error.response?.data?.message || 'বিষয় আপডেট করতে সমস্যা হয়েছে';
                        ...
                        <div className="grid grid-cols-1 gap-8">
                            ...
                            <div className="flex gap-4 pt-6 border-t">
                                ...
                                >
                                    ...
                                    {loading ? 'আপডেট হচ্ছে...' : 'বিষয় আপডেট করুন'}
                                </MainButton>
                            </div>
                        </div>
                    </form>
